  ...
  return (
    <View style={[cardStyle, { flex: 0.8, minWidth: isMobile ? 0 : 340, marginBottom: 0 }]}> 
      <SGPlanningSectionTitle
        ...
        title="4. Break-even"
        accent="#4F46E5"
        subtitle="Phân tích điểm hòa vốn"
      />
      ...
      >
        <Text style={{ color: '#4F46E5', fontSize: 10, fontWeight: '900', letterSpacing: 1.6 }}>DOANH THU HÒA VỐN</Text>
        <Text style={{ marginTop: 8, color: '#4F46E5', fontSize: isMobile ? 34 : 42, fontWeight: '900' }}>
          {fmt(metrics.breakEvenRevenue)} <Text style={{ fontSize: 16 }}>Tỷ</Text>
        </Text>
        <Text style={{ marginTop: 8, color: '#6366F1', fontSize: 12 }}>
          Mức doanh thu tối thiểu để bù đắp {fmt(metrics.totalOpexYear)} tỷ định phí năm.
        </Text>
      </View>
      ...
      >
        ...
        <Text style={{ marginTop: 8, color: metrics.safetyMargin >= 0 ? '#059669' : '#E11D48', fontSize: isMobile ? 34 : 42, fontWeight: '900' }}>
          {fmt(metrics.safetyMargin)} <Text style={{ fontSize: 16 }}>Tỷ</Text>
        </Text>
        ...
        <Text style={{ marginTop: 10, color: metrics.safetyMargin >= 0 ? '#059669' : '#E11D48', fontSize: 12, fontWeight: '700' }}>
          {metrics.safetyMargin >= 0 ? 'An toàn' : 'Rủi ro'} {fmt(Math.abs(metrics.safetyMarginPct), 1)}% so với hòa vốn.
        </Text>
      </View>
    </View>
  );
