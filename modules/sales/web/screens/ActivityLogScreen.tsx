        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-12 h-12 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center shrink-0 shadow-sm">
              ...
              <div className="min-w-0">
                <h2 className="text-[20px] font-black text-sg-heading leading-none tracking-tight whitespace-nowrap">
                  {mode === 'team' ? 'Nhật Ký Team' : 'Nhật Ký Kinh Doanh'}
                </h2>
                <p className="text-[12px] font-bold text-sg-muted uppercase tracking-wider mt-1.5 opacity-80">
                  {mode === 'team' ? 'Theo dõi dữ liệu toàn đội' : 'Dữ liệu hoạt động cá nhân'} • {filteredSummaries.length} bản ghi
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2 shrink-0">
              <button onClick={fetchActivities} className="h-11 px-4 flex items-center gap-2 bg-white/60 dark:bg-black/20 border border-sg-border rounded-xl text-sg-muted hover:text-sg-heading hover:bg-white transition-all font-bold text-[13px]" title="Làm mới">
                <RefreshCw size={16} /> <span className="hidden sm:inline">Làm mới</span>
              </button>
              <button 
                onClick={() => setIsModalOpen(true)}
                className="h-11 px-5 flex items-center gap-2 bg-linear-to-r from-emerald-500 to-emerald-600 border border-emerald-500/20 rounded-xl text-[13px] font-black text-white hover:shadow-lg hover:shadow-emerald-500/25 hover:-translate-y-0.5 transition-all shadow-md shadow-emerald-500/10"
              >
                <Plus size={18} /> <span>Nhập Hoạt Động</span>
              </button>
            </div>
          </div>

          {/* Row 2: Smart Filters Area */}
