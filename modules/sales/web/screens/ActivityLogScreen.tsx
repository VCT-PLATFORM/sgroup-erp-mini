        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[10px] font-bold text-sg-muted uppercase tracking-wider mt-0.5">
                {mode === 'team' ? 'Dữ liệu đội' : 'Cá nhân'} • {filteredSummaries.length} bản ghi
              </p>
            </div>
          </div>
          
          {/* Center: Filters (Scrollable on small screens) */}
          <div className="flex-1 flex justify-center min-w-0">
            <div className="flex items-center gap-3 overflow-x-auto no-scrollbar max-w-full pb-1">
              ...
              {mode === 'team' && (
                <>
                  <div className="w-px h-5 bg-sg-border/50 mx-1 shrink-0"></div>
                  <CustomSelect
                    value={filterTeam}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={fetchActivities} className="h-10 w-10 flex items-center justify-center bg-white/60 dark:bg-black/20 border border-sg-border rounded-xl text-sg-muted hover:text-sg-heading hover:bg-white transition-all" title="Làm mới">
              <RefreshCw size={14} />
            </button>
            <button 
              onClick={() => setIsModalOpen(true)}
              className="h-10 px-4 flex items-center gap-2 bg-linear-to-r from-emerald-500 to-emerald-600 border border-emerald-500/20 rounded-xl text-[12px] font-black text-white hover:shadow-lg hover:shadow-emerald-500/20 hover:-translate-y-0.5 transition-all shadow-emerald-500/10"
            >
              <Plus size={16} /> 
              <span className="hidden lg:inline">Nhập Hoạt Động</span>
            </button>
          </div>
        </div>
      </div>
