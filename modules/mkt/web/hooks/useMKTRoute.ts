...
export function useMKTRoute(validKeys: string[]) {
  const parseHash = () => {
    if (typeof window === 'undefined') return { key: 'MKT_DASHBOARD', params: new URLSearchParams() };
    const fullHash = window.location.hash.replace('#', '');
    const [path, queryString] = fullHash.split('?');
    const key = path.toUpperCase();
    
    return {
      key: validKeys.includes(key) ? key : 'MKT_DASHBOARD',
  ...
  const navigate = useCallback((key: string, params?: Record<string, string>) => {
    ...
    setRouteState({
      key: validKeys.includes(key.toUpperCase()) ? key.toUpperCase() : 'MKT_DASHBOARD',
      params: searchParams
    });
