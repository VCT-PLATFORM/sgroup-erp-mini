...
export function SGSkeleton({ className = '', variant = 'text', width, height, lines }: SkeletonProps) {
...
/** Skeleton for a stats card (KPI card) */
    ...
    <tr className="animate-pulse">
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="px-6 py-4">
          <div className={`h-4 rounded-lg bg-sg-border/60 dark:bg-white/6 ${i === 0 ? 'w-32' : 'w-20'}`} />
        </td>
      ))}
    </tr>
  );
...
/** Skeleton for an employee card (grid view) */
...
/** Skeleton for the dashboard panel */
        ...
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="flex justify-between items-end">
            <div className="flex-1">
              ...
              <div className="h-2 rounded-full bg-sg-border/60 dark:bg-white/6" style={{ width: `${30 + i * 15}%` }} />
            </div>
