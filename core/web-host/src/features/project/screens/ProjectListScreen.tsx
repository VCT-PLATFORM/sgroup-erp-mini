...
import { Search, Filter, MoreHorizontal, Calendar, Target, Plus, Users, Building2, MapPin } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col h-full bg-transparent relative z-10 overflow-hidden p-4 sm:p-8 lg:px-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h2 className="text-[32px] font-black text-sg-heading tracking-tight drop-shadow-md">Thiết lập Dự án mới</h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="h-12 px-6 flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 rounded-xl transition-all shadow-[0_8px_24px_rgba(6,182,212,0.25)] hover:shadow-[0_12px_32px_rgba(6,182,212,0.4)] hover:-translate-y-0.5 relative overflow-hidden group">
          ...
          {filtered.map(proj => {
            ...
            return (
              <div key={proj.id} className="relative group perspective-[1000px]">
                <div className="bg-sg-card/60 backdrop-blur-2xl rounded-[32px] border border-sg-border p-7 flex flex-col gap-6 shadow-[0_4px_24px_rgba(0,0,0,0.02)] transition-all duration-500 hover:shadow-[0_24px_48px_rgba(0,0,0,0.08)] group-hover:-translate-y-2 relative overflow-hidden">
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <button className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-sg-btn-bg text-sg-muted hover:text-sg-heading transition-colors">
                  ...
                  <div className="flex flex-col gap-3 relative z-10 mt-1">
                    ...
                    <div className="flex items-center justify-between text-[13px] font-bold text-sg-subtext bg-sg-btn-bg px-3 py-2 rounded-xl border border-sg-border">
                      <div className="flex items-center gap-2">
                         <Target size={14} className="text-indigo-400" />
                         <span>Quy mô dự án</span>
                      </div>
                      <span className="text-sg-heading font-black">{proj.totalUnits} sản phẩm</span>
                    </div>
                  </div>

                  <div className="pt-5 mt-auto border-t border-sg-border/60 flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-2">
                      <div className="w-9 h-9 rounded-[10px] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[12px] font-black shadow-sm">
                        CĐT
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[11px] font-bold text-sg-muted">Chủ đầu tư</span>
                        <span className="text-[13px] font-extrabold text-sg-heading leading-tight truncate max-w-[120px]" title={proj.developer}>{proj.developer}</span>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-3 text-sg-muted">
                      <div className="flex items-center gap-1.5" title="Mở bán dự kiến">
                        <Calendar size={14} />
                        <span className="text-[12px] font-bold">{new Date(proj.startDate).toLocaleDateString('vi-VN')}</span>
                      </div>
                    </div>
                  </div>
