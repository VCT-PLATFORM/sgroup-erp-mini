...
import { LayoutDashboard, Building2, Grid, FileText, TrendingUp, Settings } from 'lucide-react';

const MENU_ITEMS = [
  { path: 'dashboard', label: 'Dashboard BĐS', icon: LayoutDashboard },
  { path: 'list', label: 'Quản lý Dự án', icon: Building2 },
  { path: 'inventory', label: 'Rổ hàng tổng', icon: Grid },
  { path: 'legal', label: 'Tiến độ Pháp lý', icon: FileText },
  { path: 'reports', label: 'Phân tích & Báo cáo', icon: TrendingUp },
];
    ...
    <div className="w-[280px] h-full flex flex-col pt-6 pb-6 relative z-10">
      ...
      <div className="px-6 pb-8">
        <div className="flex items-center gap-4 bg-sg-bg/40 p-3.5 rounded-[20px] border border-sg-border shadow-sm">
          ...
          <div>
            <h1 className="text-[17px] font-black tracking-tight text-sg-heading drop-shadow-sm leading-none">REAL ESTATE</h1>
            <p className="text-[11px] font-extrabold text-cyan-600 dark:text-cyan-400 uppercase tracking-widest mt-1">Management</p>
          </div>
        </div>
      </div>
        ...
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3.5 rounded-[16px] transition-all duration-300 group relative overflow-hidden ${
                  isActive 
                    ? 'bg-gradient-to-r from-cyan-500/15 to-blue-600/5 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30 shadow-[0_4px_24px_rgba(6,182,212,0.15)] pointer-events-none' 
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span className={`text-[14px] tracking-wide transition-all ${isActive ? 'font-black drop-shadow-sm' : 'font-bold'}`}>{item.label}</span>
                </>
              )}
            </NavLink>
          );
